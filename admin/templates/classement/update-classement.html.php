<h1 class="h1-admin text-center">Modifier un classement</h1>
...
<form method="POST" action="">
    <div class="form-row">
        <div class="form-group col-md-6">
            ...
            <input class="form-control" type="number" name="ordre" id="ordre" value="<?php echo $classementId->getOrdre(); ?>" required>
        </div>
    </div>
    <div class="form-row">
        <div class="form-group col-md-6">
            ...
            <select class="custom-select" name="archive" id="archive" required>
                ...
                <?php foreach ($archive as $value): ?>
                        <option <?php if($classementId->getArchive() === $value->getId()): ?> selected <?php endif; ?> value="<?php echo $value->getId(); ?>"><?php echo $value->getAnnee(); ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="form-group col-md-6">
            ...
            <select class="custom-select" name="team" id="team" required>
                ...
                <?php foreach ($teams as $value): ?>
                        <option <?php if($classementId->getTeam() === $value->getId()): ?> selected <?php endif; ?> value="<?php echo $value->getId(); ?>"><?php echo $value->getNom() . ' - ' . $value->getArchive(); ?></option>
                <?php endforeach; ?>
            </select>
        </div>
    </div>
